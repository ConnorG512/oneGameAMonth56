...
public:
  Score(T score_multiplier = 1, T max_score = std::numeric_limits<T>::max())
      : score_multiplier_{score_multiplier}, max_score_{max_score}
  {
  }
  ...
  auto setMultiplier(T value) -> T { score_multiplier_ += value; }

  auto getCurrentScore() noexcept -> T { return current_score_; }

  auto getHighScore() noexcept -> T { return high_score_; }
};
} // namespace Game
